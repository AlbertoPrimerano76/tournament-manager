...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#ecfff4_0%,_#f4f7fb_40%,_#e9eef5_100%)] flex">
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 admin-sidebar text-white transform transition-transform
      ...
      `}>
        <div className="border-b border-white/10 px-6 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xs font-semibold uppercase tracking-widest3 text-emerald-100/72">Area amministrativa</p>
          ...
          <div className="mt-5 rounded-[1.4rem] border border-white/10 bg-white/5 p-4">
            <div className="flex items-center gap-2 text-emerald-100/80">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest3">Utente collegato</span>
            </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition-colors ${
                  isActive ? 'bg-white text-rugby-brand shadow-lg' : 'text-emerald-50/72 hover:bg-white/10 hover:text-white'
                }`}
              >
                {item.icon}
                {item.label}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-white/70 bg-white/70 px-4 shadow-sm backdrop-blur md:px-6">
          <button aria-label="Apri menu" className="rounded-xl p-2 hover:bg-slate-100 md:hidden" onClick={() => setSidebarOpen(true)}>
            <Menu className="h-5 w-5" />
          </button>
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-widest3 text-slate-500">{pageMeta.kicker}</p>
